Feature: Telegram Bot 接入
  ...
  @unit
  Scenario: 用户消息映射到独立的 agent thread
  ...
  Scenario: 多轮对话保持上下文
    Given Telegram bot 已初始化
    When 用户 "100" 发送消息 "我叫小明"
    And 用户 "100" 发送消息 "你还记得我叫什么名字吗？"
    Then bot 的回复应提及 "小明"
  ...
  Scenario: 不同用户对话上下文互不干扰
    Given Telegram bot 已初始化
    When 用户 "101" 发送消息 "我的名字是 Alice"
    And 用户 "102" 发送消息 "我叫什么名字？"
